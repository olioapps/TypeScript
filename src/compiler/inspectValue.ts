namespace ts {
    ...
    function getFunctionOrClassInfo(obj: AnyFunction, name: string, recurser: Recurser): ValueInfoFunctionOrClass {
        ...
        const toString = functionToString(obj);
        const source = stringContains(toString, "{ [native code] }") ? cast(safeGetPropertyOfObject(obj, "length"), isNumber) : toString;
